...
export function withCommonHeaders<T extends APIGatewayProxyHandler>(handler: T) {
    const wrapped: APIGatewayProxyHandler = (event, context, cb) => {
        return handler(event, context, () => cb(new Error("Callback style response from wrapped handler not supported"))
        )?.then((response) => {
            const origin = event.headers && Object.entries(event.headers).find(([k, v]) => k.toLowerCase() === "origin" && v)?.[1];
            const headers = {
