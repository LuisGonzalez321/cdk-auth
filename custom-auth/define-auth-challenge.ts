...
function handleMagicLinkResponse(event: DefineAuthChallengeTriggerEvent) {
    ...
    if (lastResponse.challengeResult) {
        return allow(event);
...
function countAttempts(event: DefineAuthChallengeTriggerEvent, excludeProvideAuthParameters = true) {
    if (!excludeProvideAuthParameters) return event.request.session.length;
    return event.request.session.filter((entry) =>
        entry.challengeMetadata !== "PROVIDE_AUTH_PARAMETERS").length;
}
