import {APIGatewayProxyWithCognitoAuthorizerHandler, APIGatewayProxyHandler} from "aws-lambda";
...
const allowedRelyingPartyIdHashes = allowedRelyingPartyIds.map((relyingPartyId) =>
...
if (!allowedOrigins.length) throw new Error("Environment variable ALLOWED_ORIGINS is not set");
...
type RpPublicKeyCredentialCreationOptions = Omit<PublicKeyCredentialCreationOptions, "challenge" | "user" | "excludeCredentials"> & {
    challenge: string;
    user: UserDetails;
...
type RpPublicKeyCredentialDescriptor = Omit<PublicKeyCredentialDescriptor, "id" >& { id: string };
...
async function deleteCredential({userId, credentialId}: { userId: string; credentialId: unknown}) {
...
async function storeAuthenticatorChallenge(options: RpPublicKeyCredentialCreationOptions) {
    await ddbDocClient.send(
      new PutCommand({
         TableName: process.env.DYNAMODB_AUTHENTICATORS_TABLE!,
         Item: {
...
async function requestCredentialsChallenge({userId, name, displayName, rpId,}: {
    ...
    const existingCredentials = await getExistingCredentialsForUser({userId, rpId,});
    const options: RpPublicKeyCredentialCreationOptions = {
        challenge: randomBytes(64).toString("base64url"),
        attestation: (process.env.ATTESTATION as AttestationConveyancePreference) ?? "none",
        rp: {
        ...
        authenticatorSelection: {
            userVerification: process.env.USER_VERIFICATION as UserVerificationRequirement,
            authenticatorAttachment: (process.env.AUTHENTICATOR_ATTACHMENT as AuthenticatorAttachment) || undefined,
            residentKey: (process.env.REQUIRE_RESIDENT_KEY as ResidentKeyRequirement) || undefined,
            requireResidentKey: (process.env.REQUIRE_RESIDENT_KEY && process.env.REQUIRE_RESIDENT_KEY === "required") || undefined,
        },
        timeout: authenticatorRegistrationTimeout, // 6 minutes
        excludeCredentials: existingCredentials.map((credential) => ({id: credential.credentialId, type: "public-key"})),
    };
...
function assertBodyIsObject(body: unknown): asserts body is Record<string | number, unknown> {
    if (body === null || typeof body !== "object") {
        throw new UserFacingError(`Expected body to be an object, but got ${typeof body}`);
    }
}

function assertBodyIsCredentialsResponse(body: unknown): asserts body is CompleteCreateCredentialResponse {
    assertBodyIsObject(body);
    ["attestationObjectB64", "clientDataJSON_B64", "friendlyName"].forEach((key) => {
      if (!body[key] || typeof body[key] !== "string") {
          throw new UserFacingError(`Expected ${key} to be a string, but got ${typeof body[key]}`);
      }
    });
...
async function handleCredentialsResponse(
    ...
    const { Attributes: storedChallenge } = await ddbDocClient.send(
            new DeleteCommand({
                TableName: process.env.DYNAMODB_AUTHENTICATORS_TABLE!,
                Key: {
                    pk: `USER#${userId}`,
...
function parseBody(event: { body?: string | null; isBase64Encoded: boolean }) {
    try {
        return event.body ? (JSON.parse(event.isBase64Encoded
                ? Buffer.from(event.body, "base64url").toString()
                : event.body) as unknown) : {};
    } catch (err) {
        logger.error(err);
